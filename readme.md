...
A client sends a lead to the FastAPI API.
The API stores the lead in MySQL.
A processing task is created and pushed into Redis.
A worker continuously listens to the queue.
The worker processes the lead using OpenAI.
The lead is classified as hot, warm, or cold.
Task status is updated in the database.
A webhook notification is sent after processing completes.
